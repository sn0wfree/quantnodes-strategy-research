...
简略版（注册时 inspect 生成 → 预置 prompt，轻量目录）：
  name / 用途一句话 / 必填参数(名+类型) / 类别 / 副作用(effects)

详细版（tool_help(name) 实时读源码 → 按需返回）：
  # <name> 工具说明书
  用途        完整用途 + 适用场景（何时用/何时不用）
  参数        每个参数：语义 / 类型 / 默认值 / 可选性 / 示例值
  示例        1-2 个最小可用调用示例（JSON）
  边界        前置条件、上下文安全说明、行为细节
  错误处理范式  各失败场景的错误结构：received/expected/fix 含义、
               workflow 提示、重试/幂等建议
  相关工具     与之衔接的工具（可选）
```

- docstring 约定节：`参数:` / `示例:` / `边界:` / `错误处理范式:` / `相关工具:`
- 组合工具同机制：说明书两版从组合配置（steps + 映射符号）生成
...
| 阶段 | 内容 | 状态 |
|---|---|---|
| P1 | 说明书基础设施：模板规范 + 注册时解析器（简略版）+ `tool_help` 工具 | 待实施 |
