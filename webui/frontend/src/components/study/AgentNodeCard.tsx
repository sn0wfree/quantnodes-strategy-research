...
export function AgentNodeCard({ agent, status, onClick }: AgentNodeCardProps) {
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className={`flex w-32 flex-shrink-0 flex-col rounded-xl border p-3 transition-all hover:scale-[1.02] hover:shadow-lg ${config.border} ${config.bg} ${status?.status === 'running' ? 'animate-pulse' : ''}`}
